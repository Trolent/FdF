...
/*   Updated: 2024/02/21 11:20:41 by trolland         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
const int	z_key(t_vars *vars)
{
	...
	while(i < vars->map->rows)
	{
		...
		while(j < vars->map->columns)
		{
			...
			vars->map->coord[i][j].x[ISO] = vars->map->coord[i][j].x[ISO] + vars->map->mid[X];
			vars->map->coord[i][j].y[ISO] = vars->map->coord[i][j].y[ISO] + vars->map->mid[Y];
			j++;
		}
		i++;
	}
